<div>
    <div
        class="@if ($isDrawer) p-4 h-full flex flex-col @else max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 min-h-[70vh] mt-20 @endif">
        @if (!$isDrawer)
        <div class="text-sm breadcrumbs mb-6">
            <ul>
                <li><a href="{{ route('home') }}">Home</a></li>
                <li><a  class="text-primary">Cart</a></li>
            ...
            <div
                ...
                <div class="@if (!$isDrawer) divide-y divide-gray-100 bg-base-200 @endif">
                    @foreach ($cartItems as $key => $item)
                        <div
                            class="@if ($isDrawer) bg-base-200 border-b-1 border-accent-content  p-4 @else p-6 hover:bg-base-100 transition-colors duration-200 @endif">
                            <div class="flex gap-4">
                                ...
                                <div class="flex-shrink-0">
                                    <div
                                        ...
                                        @if ($item->product->isSimple())
                                            @if ($item->product->hasMedia('main_image'))
                                        ...
                                        @elseif($item->product->hasColorsOnly() || $item->product->hasColorsAndSizes())
                                            @if ($item->productColor->hasMedia('color_images'))
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex flex-col h-full">
                                        ...
                                        <div class="flex-1">
                                            <div
                                                class="flex @if ($isDrawer) flex-col items-center @else justify-between @endif">
                                                <div>
                                                    <h3
                                                        class="@if ($isDrawer) text-base font-bold text-neutral line-clamp-1 @else text-lg font-semibold text-neutral-content @endif">
                                                        @if (!$isDrawer)
                                                            <a href="{{ route('product.show', [
                                                                'categoryPath' => $item->product->category->full_path,
                                                ...
                                                <div class="text-right">
                                                    <p
                                                        class="@if ($isDrawer) text-sm font-semibold text-neutral @else text-lg font-bold text-neutral @endif">
                                            ...
                                            <div
                                                ...
                                                @if ($item->product->hasColorsOnly() || $item->product->hasColorsAndSizes())
                                                    <div
                                                        ...
                                                        <span class="w-2 h-2 rounded-full mr-1.5"
                                                            style="background-color: {{ $item->productColor->hex_code ?? '#6b7280' }}"></span>
                                                    ...
                                                    <div class="text-sm text-gray-600">
                                                        ...
                                                        @if ($item->product->hasActiveDiscount() && !$isDrawer)
                                                            <div class="flex justify-between gap-4">
                                                                ...
                                                                <p class="text-sm text-gray-500">Save: EGP
                                                                    {{ number_format($item->discount_amount, 2) }}</p>
                                                            </div>
                                                        @endif

                                                        <p class="text-xs text-green-500">
                                                            @if ($item->product->isSimple())
                                                                Stock: {{ $item->product->simple_quantity }}
                                                            @endif
                                                            @if ($item->product->hasColorsOnly())
                                                                Stock: {{ $item->productColor->total_quantity }}
                                                            @endif
                                                            @if ($item->product->hasColorsAndSizes())
