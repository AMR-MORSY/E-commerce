...
<div class="max-w-7xl mx-auto px-4 py-8">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="space-y-4">
            ...
            <div class="relative bg-gray-100 rounded-lg overflow-hidden aspect-square">
                @if($currentImages->isNotEmpty())
                    <img src="{{ $currentImages[$currentImageIndex]->getUrl('large') }}" 
                         alt="{{ $product->name }}"
                         class="w-full h-full object-cover">
        ...
        <div class="space-y-6">
            <!-- Product Info -->
            <p class=" text-3xl  text-amber-950">{{$product->name}}</p>
           <!-- Price Section -->
            ...
            @else
                ...
                @if($selectedSize && $selectedSize->price_adjustment != 0)
                    <div class="text-sm text-gray-500">
                        Base: ${{ number_format($product->base_price, 2) }}
                    </div>
                @endif
            @endif
        </div>
                ...
                <div class="flex flex-wrap gap-3">
                    @foreach($product->colors as $color)
                        <button wire:click="selectColor({{ $color->id }})"
                            ...
                            <!-- Color Image -->
                            <div class="w-20 h-20 rounded-lg overflow-hidden border-2 transition {{ $selectedColor && $selectedColor->id === $color->id ? 'border-blue-500 ring-2 ring-blue-200' : 'border-gray-200 hover:border-gray-400' }}">
                            ...
                            <!-- Color Name Tooltip -->
                            <div class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 text-white text-xs rounded whitespace-nowrap opacity-0 group-hover:opacity-100 transition pointer-events-none">
                                {{ $color->name }}
                            </div>
...
{{--                                 
                    ...
                    @if($selectedSize)
                        <div class="mt-2 text-sm text-gray-600">
                            @if($selectedSize->quantity < 5)
                                <span class="text-orange-600 font-medium">Only {{ $selectedSize->quantity }} left in stock</span>
                            @else
                ...
                <div>
                    ...
                    <div class="flex items-center gap-3">
                        <button wire:click="decrementQuantity"
                                ...
                                class="w-10 h-10 border rounded-lg flex items-center justify-center hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition">
                        ...
                        <input type="number" 
                               ...
                               class="w-20 text-center border rounded-lg px-3 py-2 font-medium">
                        
                        <button wire:click="incrementQuantity"
                                @if($quantity >= $selectedSize->quantity) disabled @endif
                                class="w-10 h-10 border rounded-lg flex items-center justify-center hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed transition">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <!-- Add to Cart Button -->
            <button wire:click="addToCart"
                    @if(!$selectedColor || !$selectedSize || $selectedSize->quantity === 0) disabled @endif
                    class="w-full bg-blue-600 hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-semibold py-4 rounded-lg transition">
    ...
    <div id="zoom-modal" class="hidden fixed inset-0 bg-black/90 z-50 flex items-center justify-center p-4"
        ...
        @if($currentImages->isNotEmpty())
            <img src="{{ $currentImages[$currentImageIndex]->getUrl('zoom') }}" 
                 alt="{{ $product->name }}"
